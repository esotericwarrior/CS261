...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	assert(val != 0);			// Verify that the value being passed isn't 0 (pre-condition).
	if (cur == 0) {	// If current value == 0 (null), it doesn't exist yet, so we:
		newNode = (struct Node *) malloc(sizeof(struct Node));	// Create the new node.
		assert(newNode != 0);	// Verify the node was successfully created.
		newNode->val = val;		// Assign the passed value to the newly created node.
		newNode->left = 0;
		newNode->right = 0;
		return newNode;
	}
...
{
	/*write this*/
	assert(tree != 0);	// Pre-condition: Verify that the tree isn't null.
	assert(val != 0);	// Pre-condition: Verify that the value isn't null.
	struct Node *iterator = tree->root; // Declare new root.
	while (iterator != 0) {	// Begin loop:
...
{
	/*write this*/
	assert(cur != 0);	// Pre-condition: Verify that current isn't null.
	while (cur->left != 0) {
		cur = cur->left;
	}
	return cur->val;
...
{
	/*write this*/
	assert(cur != 0);	// Pre-condition: Verify that current in not null.
	if (cur->left != 0) {

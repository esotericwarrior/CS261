...
{
	// FIXME: you must write this
	//s->structSize = 0;		// Set the initial size of the stack to 0.
	//						// allocate memory for the listQueues in linkedListStack s
	...
	// call the function listQueueInit() by passing the s->Q2
	listQueueInit(s->Q1);
	listQueueInit(s->Q2);
}
...
void linkedListStackPush(struct linkedListStack *s, TYPE d) {
    ...
    // FIXME: you must write this
	// call the functionlistQueueAddBack() by passing
	// s->Q2 and d
	...
	// swap the queues
	struct listQueue *q = s->Q1;	// swap the names of two queues
	s->Q1 = s->Q2;
	s->Q2 = q;
...
void linkedListStackPop(struct linkedListStack *s) {
	...
	if (!listQueueIsEmpty(s->Q1)) {// If Q1 is empty, return:
		listQueueRemoveFront(s->Q1);//remove front element from q1
		s->structSize--;	//decreasing the size of our stack on pop
	}
	else {
...
TYPE linkedListStackTop(struct linkedListStack *s) {
	...
	//}
	//else {
	//	return listQueueIsEmpty(s->Q1);	// Returning the front of Q1 which will be the top of the stack.
	//}
	...
	if (listQueueIsEmpty(s->Q1)) {	// If no elements are in Q1, return:
		...
		// get the front value of s->Q1 and store it into the TYPE value
